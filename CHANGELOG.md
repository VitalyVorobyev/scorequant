...
- `retention_uncertainty` and `RetentionUncertainty` (ADR 0039): the audited plug-in standard error
  ...
  equally weighted sample of true scores. A rank-deficient full moment, a rank-deficient between-cell
  moment and influence values that cancel to rounding each return a named status with the interval
  withheld. Sampling uncertainty only; proxy-score bias is documented as a separate quantity.
