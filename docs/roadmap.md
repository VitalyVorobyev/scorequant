...
| D — Frozen-rule uncertainty | Scalar O6 proved and audited (PR #53); vector O7 proved, audit pending (PR #59). The remaining research is the finite closure programme in `agenticresearch/OPEN_PROBLEMS.md`. | One conditional scalar retention interval result or precise obstruction, plus one seeded experiment. Independent audit before promotion. No public estimator yet. |
...
| G — Research Atlas | The research surface is generated from the registry export as an explorable atlas (ADR 0033): entity pages, argument map, landscape, frontier, literature, machine-checked chain, library relation. The public edge typing anticipates F without touching the registry schema. | `tests/test_atlas_data.py`, website unit and e2e checks pass; the owner reviews the withheld list and the headline choices. Internal vocabulary in registry statements is edited upstream as claims are touched. |
