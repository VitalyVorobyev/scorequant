 ...
 */
export default function LandscapePage({core}: AtlasPageProps<Record<string, never>>): React.JSX.Element {
  ...
  return (
    <AtlasShell
      description="Every result in the research record placed at the criterion it is stated for and the level of the problem it settles."
      title="The landscape"
      wide
    >
      <div className="atlas__measure">
        ...
        <p className="atlas__lead">
          A result is stated for a criterion, which is what the binning is asked to maximise, and at a level of the problem: a labelling of one
          fixed table of scores, a rule fitted on a sample and applied to observations not yet seen, or the design under the score law itself.
          The grid places every result at that pair.
        </p>
        <p>
          A result stated for several criteria appears in each of their rows, so a glyph is a placement rather than a result. Verification
          records are left out, since they carry no result of their own. An empty cell is territory nobody has worked: sometimes because the
          question does not arise there, sometimes because no one has asked it.
        </p>
      </div>
      ...
      <div aria-live="polite" className="atlas__measure landscape__detail">
        {selectedCriterion && selectedLevel ? (
          <>
            ...
            <EntityList core={core} empty="Nothing stands here yet." ids={selectedIds} />
          </>
        ) : (
